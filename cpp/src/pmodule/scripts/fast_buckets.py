...
def exact_buckets_2(A, count, k):
    ...
    # The base case of the dynamic program
    for i in range(n):
        err[i, 0] = np.sum(vv[:i+1]) - np.sum(v[:i+1])**2/np.sum(count[:i+1])
        borders[i, 0] = 0  # with one bucket the last bucket starts at zero.
    ...
    for p in range(1, k):
        for i in range(n):
            cur_err = err[i, 0]
            for x in range(1, i + 1):  # x is a candidate for creating a new bucket [x....i]
                last_bucket = np.sum(vv[x:i+1]) - np.sum(v[x:i+1])**2/np.sum(count[x:i+1])
                tmp = err[x - 1, p - 1] + last_bucket 
                if tmp <= cur_err:
                    cur_err = tmp
                    borders[i, p] = x
            err[i, p] = cur_err
